...
const BookCard = ({ book, onSelect, onAddToCart }) => {
  ...
  return (
    <div className="glass-panel" style={{ padding: "20px", display: "flex", flexDirection: "column", gap: "16px", cursor: "pointer" }} onClick={() => onSelect(book)}>
      ...
      <div style={{ 
        height: "200px", 
        background: book.image ? `linear-gradient(rgba(0,0,0,0.2), rgba(0,0,0,0.8)), url(${book.image}) center/cover no-repeat` : book.gradient, 
        borderRadius: "12px", 
        display: "flex", 
        ...
        <span style={{ fontSize: "2.5rem", alignSelf: "flex-end", opacity: book.image ? 0 : 1 }}>{book.icon}</span>
        <div>
          ...
          <h4 style={{ fontSize: "1.1rem", fontWeight: "800", marginTop: "8px", overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: "2", WebkitBoxOrient: "vertical", lineHeight: "1.2", textShadow: book.image ? "0 2px 4px rgba(0,0,0,0.8)" : "none" }}>{book.title}</h4>
        </div>
      </div>
